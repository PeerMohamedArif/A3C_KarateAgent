...
class PreprocessAtari(ObservationWrapper):
  ...
  def observation(self, img):
    img = self.crop(img)
    img = cv2.resize(img, self.img_size)  # Resize to 42x42
    if not self.color:
        if len(img.shape) == 3 and img.shape[2] == 3:
            img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # Convert to grayscale
    img = img.astype('float32') / 255.  # Normalize pixel values

    # Ensure correct stacking of frames
    if self.color:
        self.frames = np.roll(self.frames, shift=-3, axis=0)
    else:
        self.frames = np.roll(self.frames, shift=-1, axis=0)

    # Assign the new frame to the last position
    if self.color:
        self.frames[-3:] = img
    else:
        self.frames[-1] = img
...
class Agent():
  ...
  def step(self, state, action, reward, next_state, done):
    ...
    action = torch.tensor(action, dtype=torch.long, device=self.device)  # Ensure tensor type
...
def evaluate(agent, env, n_episodes=1):
    ...
    for _ in range(n_episodes):
        ...
        # Ensure state is already in grayscale (stacked 4 frames)
        if len(state.shape) == 3 and state.shape[0] != 4:  # If it's a single grayscale frame, stack it
            state = np.expand_dims(state, axis=0)  # Add channel dimension
            state = np.repeat(state, 4, axis=0)  # Stack 4 grayscale frames

        total_reward = 0
        done = False

        while not done:
            action = agent.act(state)[0]  # Get action from the agent
            next_state, reward, done, _, _ = env.step(action)
            total_reward += reward

            # Ensure next_state is grayscale and resized properly
            if len(next_state.shape) == 3 and next_state.shape[0] != 4:
                next_state = np.expand_dims(next_state, axis=0)  # Add channel dimension
                next_state = np.repeat(next_state, 4, axis=0)  # Stack 4 grayscale frames

            # Stack frames correctly
            next_state_stack = np.roll(state, shift=-1, axis=0)  # Shift old frames
            next_state_stack[-1] = np.squeeze(next_state[0])  # Update last frame
            state = next_state_stack  # Update state
...
def show_video_of_model(agent, env_name):
    ...
    # Preprocess first state (Convert to grayscale & resize)
    state = cv2.cvtColor(state, cv2.COLOR_RGB2GRAY)  # Convert to grayscale
    state = cv2.resize(state, (42, 42)) / 255.  # Resize and normalize
    state = np.expand_dims(state, axis=0)  # Add channel dimension
    state = np.repeat(state, 4, axis=0)  # Stack 4 grayscale frames
    ...
    while not done:
        ...
        action = agent.act(state)[0]  # Ensure single action
        next_state, reward, done, _, _ = env.step(action)

        # Preprocess next state
        next_state = cv2.cvtColor(next_state, cv2.COLOR_RGB2GRAY)  # Convert to grayscale
        next_state = cv2.resize(next_state, (42, 42)) / 255.
        next_state = np.expand_dims(next_state, axis=0)

        # Stack frames correctly
        next_state_stack = np.roll(state, shift=-1, axis=0)  # Shift old frames
        next_state_stack[-1] = np.squeeze(next_state)  # Update last frame
        state = next_state_stack  # Update state
